...
userRouter.get("/requests/pending",userAuth,async(req,res)=>{
    ...
    const requests= await connectionRequest.find({
        ...
        status: "interest"
    }).populate("senderUserId",["firstName", "lastName","photoUrl", "age", "gender", "about","skills"]);
...
userRouter.get("/users/connections",userAuth,async(req,res)=>{
    try {
        ...
        .populate("recieverUserId",["firstName", "lastName","photoUrl", "age", "gender", "about","skills"]);

        const data = connections.map((connection)=>{
            if(connection.senderUserId?._id.toString()===loggedUser._id.toString()){
...
userRouter.get("/feed",userAuth,async(req,res)=>{
    try {
        ...
        }).select(["firstName", "lastName", "emailId", "skills", ]).skip(startIndex ).limit(limit)
